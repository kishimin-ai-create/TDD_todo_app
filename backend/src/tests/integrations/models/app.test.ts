...
describe('AppEntity', () => {
  ...
  it('entity created by interactor has all AppEntity fields with correct types', async () => {
    ...
    const app = await interactor.create({ userId: USER_ID, name: 'Shape Test' });
  ...
  it('soft-deleted entity returned by interactor has deletedAt set to a string', async () => {
    ...
    const app = await interactor.create({ userId: USER_ID, name: 'Del Test' });
    const deleted = await interactor.delete({ userId: USER_ID, appId: app.id });
