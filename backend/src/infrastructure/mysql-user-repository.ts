...
export function createMysqlUserRepository(pool: MysqlPool): UserRepository {
  async function save(user: UserEntity): Promise<void> {
    try {
      await pool.execute(
        ...
        [user.id, user.email, user.passwordHash, new Date(user.createdAt), new Date(user.updatedAt)],
      );
    } catch (err: unknown) {
      logError('save', err);
  ...
  async function findByEmail(email: string): Promise<UserEntity | null> {
    try {
      const [rows] = await pool.execute<UserRow[]>(
        'SELECT id, email, passwordHash, createdAt, updatedAt FROM User WHERE email = ?',
        [email],
      );
      const row = rows[0];
  ...
  async function findById(id: string): Promise<UserEntity | null> {
    try {
      const [rows] = await pool.execute<UserRow[]>(
        'SELECT id, email, passwordHash, createdAt, updatedAt FROM User WHERE id = ?',
        [id],
      );
      const row = rows[0];
      return row ? rowToUser(row) : null;
    } catch (err: unknown) {
      logError('findById', err);
      throw new AppError('REPOSITORY_ERROR', 'Repository operation failed', { cause: err });
    }
  }
